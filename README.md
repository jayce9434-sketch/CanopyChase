...
## SMART CHAT + MORSE V5
- Fixed chat intent matching so `what` no longer accidentally matches `hat`/cosmetic replies.
- Bots now prioritize the meaning of statements, emotions, plans, opinions, questions, and follow-up context before game-info answers.
- Statements such as “I’m leaving if Morse joins” get direct contextual replies instead of an explanation of Morse.
- Bot promises trigger actual movement/follow/travel behavior.
- Morse cave roll remains 6.66% per fresh cave entry.
- Morse now targets bots most of the time, actively cycles Kick Gun, Ban Gun, Ghost Monke, Invisible ambushes, stalking and scary soundboard actions, with 2-minute Morse bans.
