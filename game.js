...
function respondersForText(text){
  ...
  const sorted=[...here].sort((a,b)=>Math.abs(Math.hypot(a.x-player.x,a.y-player.y)-330)-Math.abs(Math.hypot(b.x-player.x,b.y-player.y)-330));
  return sorted.slice(0,chance(.24)?2:1);
}
function playerSpeak(text){
  text=fmt(text).slice(0,100);if(!text)return;lastPlayerSpeakAt=time;player.chat=text;player.chatUntil=time+4.5;
...
function spawnMorse(){const p=safeSpawn(save.map);morse={x:p.x,y:p.y,vx:0,vy:0,r:28,color:'#000000',visible:true,ghost:true,nextAction:time+rand(4,8),nextSound:time+rand(2,5),target:null};botsHere().forEach(b=>{b.fear=1.2;b.state='flee';botSpeak(b,pick(['MORSE IS HERE','NOPE NOPE NOPE','WHY IS MORSE HERE 😭']))});toast('... --- ...');morseBeep();}
...
function finishBotTravel(b){
  ...
  if(to===save.map&&time>b.chatUntil&&chance(.72))botSpeak(b,pick([`im here 😭`,`made it to ${MAPS[to].name.toLowerCase()}`,`okay I got here`]));
}
function updateBots(dt){
  for(const b of bots){
    if(!b.map)b.map=save.map;if(!b.memory)b.memory={lastTopic:'lobby',lastPlayerLine:'',lastPromise:''};
