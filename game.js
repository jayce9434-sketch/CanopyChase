...
const player={
  ...
  mods:{morseForm:false,longArms:false,platforms:false,ghost:false,invisible:false,kickGun:false,banGun:false,soundboard:false}
};
...
let platforms=[],bots=[],roundTimer=0,activeGlowFraction=0,morse=null,morseRollDone=false,playerMorseNextSound=0;
...
function analyzeUtterance(text,bot=null){
  ...
  const saysLeave=/\b(i'?m|i am|i'?ll|i will|im gonna|i am gonna|i'?m going to|i am going to|ima|imma|i'?ma)\s+(leave|leaving|go|going|run|running|get out|getting out|dip|dipping|bounce|bouncing)\b|\b(i gotta|i have to|i need to|i wanna|i want to)\s+(leave|go|get out|dip|run)\b/.test(s);
  const condition=/\b(if|when|whenever)\b/.test(s);
  const joins=/\b(join|joins|joined|show up|shows up|showing up|spawn|spawns|spawned|appear|appears|appeared|come|comes|arrive|arrives|get here|gets here|enter|enters)\b/.test(s);
...
function knowsGameAnswer(bot,text,intent){
  ...
  if(/long arms|platforms|ghost monke|invisible monke|kick gun|ban gun|mod menu|thumb menu/.test(s))return'press the thumb button for your sandbox mod menu: Morse Form, Long Arms, Platforms, Ghost Monke, Invisible Monke, Kick Gun, Ban Gun, and the soundboard. Kick replaces a bot. Ban removes that bot for 4 real minutes.';
...
function extractThingAfter(text,patterns){
  const s=cleanSpeech(text);
  for(const re of patterns){const m=s.match(re);if(m&&m[1])return m[1].trim().replace(/[?.!,]+$/,'');}
  return'';
}
function conversationalReply(bot,text,intent){
  const s=cleanSpeech(text),mem=bot.memory||{},here=botMap(bot),v=bot.personality||'friendly';
  const knows=!!bot.knowsMorse, morseHere=!!morse&&here==='cave';

  // Follow-up words should answer the previous conversational context, not start a random topic.
  if(/^(yes|yeah|yep|yup|sure|okay|ok|bet|alright|do it|lets do it|let's do it)$/.test(s)){
    const last=(mem.lastBotLine||'').toLowerCase();
    if(/follow|stay with|come with/.test(last)){bot.followPlayer=true;bot.followUntil=time+180;return'yeah, im with you. lead the way.';}
    if(/leave|go somewhere|move maps/.test(last)){const a=chooseBotPlan(bot,false);return a.line||'okay, lets move.';}
  }
  if(/^(no|nope|nah|not really|dont|don't)$/.test(s)){
    const last=(mem.lastBotLine||'').toLowerCase();
    if(/follow|stay with|come with/.test(last)){bot.followPlayer=false;bot.followUntil=0;return'okay, I wont follow you.';}
    if(/leave|go somewhere|move maps/.test(last))return'okay, we can stay here.';
  }

  // Exact social statements and feelings.
  if(/\b(i'?m|i am|im) (really |so |pretty |kinda |kind of )?(scared|afraid|terrified|nervous|creeped out|freaked out|spooked)\b/.test(s)){
    bot.fear=Math.max(bot.fear,morseHere?1.5:.45);
    if(morseHere){bot.knowsMorse=true;if(chance(.7)&&!bot.travel)sendBotToMap(bot,'forest','player is scared and Morse is here');return pick(['ME TOO. Morse is actually here — MOVE 😭','yeah, im scared too. stay near the entrance and dont stop moving.','I get it. those beeps are right here. im getting out of this cave.']);}
    if(here==='cave')return pick(['yeah, I get it. this cave is creepy even before anything happens.','same a little 😭 stay near me if you want.','I hear you. we can leave the cave if you want.']);
    return pick(['I hear you. wanna stay together for a bit?','yeah, I get that. I can follow you if you dont wanna be alone.','okay. im here with you — what scared you?']);
  }
  if(/\b(i'?m|i am|im) (not scared|not afraid|fine|okay)\b/.test(s)&&/morse|him|ghost|cave/.test(s)){
    if(!knows)return'fair. I still dont really know who Morse is.';
    return v==='competitive'?pick(['okay 😭 then YOU can stand closest to him','bet. im still not volunteering for the ban gun']):pick(['you can be brave, im still running if he appears 😭','fair, but im not trusting the disappearing thing.']);
  }
  if(/\b(i'?m|i am|im) (leaving|going|dipping|getting out|running)\b/.test(s)&&/\b(if|when)\b/.test(s)&&/morse|him/.test(s)){
    if(morseHere){bot.knowsMorse=true;if(!bot.travel)sendBotToMap(bot,'forest','leaving with player because Morse is here');return'HE IS HERE NOW 😭 im leaving with you.';}
    if(knows)return pick(['same. if Morse joins, im gone too.','yeah. the second I hear those beeps, im leaving.','deal. if Morse appears, we both get out.']);
    return pick(['okay 😭 I dont know Morse yet, but if you run im probably running too.','wait, who is Morse? if youre leaving over him, that sounds bad.']);
  }
  if(/\b(i'?m|i am|im) (back|here again)\b/.test(s))return pick([`yo ${save.playerName}, welcome back`,`youre back 😭 what happened?`,`hey, youre back!`]);
  if(/\b(i got|i was|ive been|i've been) (kicked|banned|tagged)\b/.test(s)){
    if(/banned/.test(s))return pick(['NOOO 😭 was that Morse?','you got banned?? how much time is left?','thats brutal. at least the timer keeps counting offline.']);
    if(/kicked/.test(s))return pick(['BRO 😭 who kicked you?','you got kicked?? did Morse do it?','nahhh 😭 welcome back.']);
    return pick(['NOOO 😭 who tagged you?','thats rough. we need a better juke next time.']);
  }

  // Direct Morse conversation. Read the verb/opinion instead of dumping lore.
  if(/\b(do you know|you know|heard of|have you seen|seen)\b.*\bmorse\b|\bmorse\b.*\b(do you know|you know)\b/.test(s)){
    if(morseHere){bot.knowsMorse=true;return pick(['YES. hes literally here right now 😭','I know Morse and I can see him. RUN.']);}
    return knows?pick(['yeah. I know Morse. I wish I didnt 😭','yeah, I know who you mean. the black 000 ghost.']):pick(['no, who is Morse?','I keep hearing that name but I dont know him.']);
  }
  if(/\bwhere (is|did|does).*morse|\bwhere'?s morse\b/.test(s)){
    if(morseHere)return'hes in Dark Caves RIGHT NOW. dont stand still.';
    if(knows)return'he only shows up in Dark Caves. I dont see him right now.';
    return'I dont know. I dont even know what Morse looks like yet.';
  }
  if(/\bwhat (does|can|did).*morse|\bwhat can morse do\b/.test(s)){
    if(!knows)return'I dont know yet. youve mentioned Morse, but I havent seen him.';
    return pick(['he can disappear, ghost through stuff, use kick and ban guns, and blast those creepy sounds.','Ghost Monke, invisibility, Morse-code beeps, kick gun, ban gun. thats why I run 😭']);
  }
  if(/\bmorse\b/.test(s)&&/\b(scary|creepy|terrifying|dangerous)\b/.test(s)){
    if(!knows)return pick(['you keep saying hes scary and I STILL dont know who he is 😭','okay now youre making me nervous. who is Morse?']);
    const neg=/\b(not|isnt|isn't|aint|ain't)\b/.test(s);
    if(neg)return v==='competitive'?pick(['maybe not to you 😭 im still not testing his ban gun','you can say that. im staying away anyway.']):pick(['nah I disagree 😭 the vanishing is enough for me','not scary?? the random beeps alone get me.']);
    bot.fear=Math.max(bot.fear,.7);return pick(['YES 😭 thats what im saying','exactly. the disappearing is the worst part.','yeah. I do not trust him at all.']);
  }
  if(/\bmorse\b/.test(s)&&/\b(here|joined|spawned|appeared|right there|behind|coming)\b/.test(s)){
    bot.knowsMorse=true;bot.fear=Math.max(bot.fear,1.7);bot.state='flee';if(here==='cave'&&chance(.75)&&!bot.travel)sendBotToMap(bot,'forest','Morse warning');
    return morseHere?pick(['I SEE HIM. RUN 😭','MORSE IS HERE. GET OUT OF THE CAVE.','NOPE NOPE NOPE, MOVE 😭']):pick(['WHERE?? I dont see him yet.','dont do that to me 😭 where is he?']);
  }

  // Opinions: answer the thing the player actually named.
  let thing=extractThingAfter(text,[/\bi (?:really )?(?:like|love) (.+)/,/\bi think (.+?) is (?:cool|good|fun|awesome)/]);
  if(thing){
    if(/morse/.test(thing))return knows?pick(['you LIKE Morse?? 😭 okay thats wild','I absolutely do not agree but I respect the confidence 😭']):'I cant judge, I dont know Morse yet.';
    if(/cave/.test(thing))return pick(['yeah, the cave atmosphere is really good.','same — creepy, but fun.']);
    if(/forest/.test(thing))return pick(['same. Forest is probably the best place to just run around.','W. Forest has the best movement space.']);
    return pick([`yeah, I get why you like ${thing}.`,`fair 😭 ${thing} is pretty good.`]);
  }
  thing=extractThingAfter(text,[/\bi (?:really )?(?:hate|dislike|dont like|don't like) (.+)/]);
  if(thing){
    if(/morse/.test(thing))return knows?'same 😭 I do NOT want him near me.':'I might agree once I actually meet him.';
    return pick([`fair. I can see why you dont like ${thing}.`,`yeah, ${thing} can get annoying.`]);
  }

  // Direct questions about this bot's current intentions/state.
  if(/\b(are you|you) (coming|following me|with me)\b|\bwill you (come|follow)\b/.test(s))return bot.followPlayer?'yeah, im following you right now.':'not right now. tell me to follow you and I will.';
  if(/\bwhy are you (following|chasing|running|leaving|scared|waiting)\b/.test(s)){
    if(bot.followPlayer)return'because you asked me to follow you.';
    if(bot.travel)return`because im heading to ${MAPS[bot.travel.to].name.toLowerCase()}.`;
    if(bot.state==='flee'&&morseHere)return'because Morse is in the cave and I am NOT staying near him 😭';
    if(bot.state==='flee')return'because something scared me and im trying to get some distance.';
    if(bot.waitUntil>time)return'because im waiting here like you asked.';
    if(bot.tagger||bot.infected)return'because im the tagger right now. im supposed to chase people 😭';
  }
  if(/\bwhere are you\b/.test(s))return bot.travel?`im on the way to ${MAPS[bot.travel.to].name.toLowerCase()}.`:`im in ${MAPS[here].name.toLowerCase()} right now.`;
  if(/\bwhat are you doing\b|\bwyd\b/.test(s))return`right now im ${currentBotActivity(bot)}.`;

  // Basic person-to-person language.
  if(/\b(youre|you're|you are) (cool|nice|awesome|funny|good)\b/.test(s))return pick(['thanks 😭 youre cool too','W, thank you','okay that actually made me happy 😭']);
  if(/\b(youre|you're|you are) (dumb|stupid|annoying|bad)\b/.test(s))return v==='chaotic'?pick(['BRO 😭 what did I do','nahhh thats crazy 😭']):pick(['dang 😭 what did I do?','okay, what am I doing wrong?']);
  if(/\b(i agree|exactly|thats true|that's true|true)\b/.test(s))return pick(['yeah exactly','thats what im saying 😭','RIGHT']);
  if(/\b(i disagree|thats wrong|that's wrong|no way)\b/.test(s))return pick(['fair 😭 what do you think instead?','okay, why do you disagree?','yeah, I could be wrong.']);

  // If the user states a game-domain fact, acknowledge THAT topic and ask only a relevant follow-up.
  if(!isInfoQuestion(text)){
    if(/\bmorse\b/.test(s))return knows?pick(['yeah, im listening. what happened with Morse?','okay, I know who you mean. what did he do?']):pick(['I heard you. I just dont know Morse yet — what happened?','okay, youre talking about Morse. who is he?']);
    if(/\bcave\b/.test(s))return pick(['yeah, im listening — what happened in the cave?','okay. what about the cave?']);
    if(/\bforest\b/.test(s))return pick(['yeah, Forest — what happened there?','okay, im with you. what about Forest?']);
    if(/\btagged|tagger|infection\b/.test(s))return pick(['NOOO 😭 what happened?','yeah, tell me what happened in the round.']);
  }
  return null;
}
function responseFor(bot,text,intent,action=null){
  ...
  const conversation=conversationalReply(bot,text,intent);if(conversation)return conversation;
...
$$('[data-mod]').forEach(b=>b.addEventListener('click',()=>{
  ...
  if(m==='morseForm'){playerMorseNextSound=time+.25;if(player.mods.morseForm){toast('BE MORSE • ON • 000');morseBeep();scarySound('drone',true);botsHere().forEach((bot,i)=>{bot.knowsMorse=true;bot.fear=Math.max(bot.fear,1.35);bot.state='flee';setTimeout(()=>{if(bots.includes(bot)&&botMap(bot)===save.map)botSpeak(bot,pick(['WAIT YOU TURNED INTO MORSE?! 😭','NOPE NOPE NOPE WHY ARE YOU MORSE','BRO TURN THAT OFF 😭','THATS MORSE 000, RUN']),chance(.35));},i*120);});}else toast('BE MORSE • OFF');}
...
function spawnMorse(){
  ...
  morse={x:p.x,y:p.y,vx:0,vy:0,r:28,color:'#000000',visible:true,ghost:true,nextAction:rand(1.15,2.4),nextSound:rand(.35,1.2),target:null,lastTarget:null,targetUntil:0,ghostUntil:0,invisibleUntil:0,shell:null,beam:null,stalkUntil:0,flash:0};
  const caveBots=botsHere('cave');caveBots.forEach((b,i)=>{panicForMorse(b,90+i*rand(70,180));if(chance(.46))setTimeout(()=>{if(bots.includes(b)&&botMap(b)==='cave'&&!b.travel)sendBotToMap(b,'forest','panic escape from Morse');},rand(900,2400));});
...
function chooseMorseTarget(){
  ...
  let pool=here;
  if(morse?.lastTarget&&pool.length>1)pool=pool.filter(b=>b!==morse.lastTarget);
  let target;if(pool.length&&chance(.84))target=pick(pool);else target=player;
  if(morse){morse.lastTarget=morse.target||morse.lastTarget;}
  return target;
}
...
function morseStalkTarget(target){
  if(!morse||!target)return;const ang=rand(0,TAU),dist=rand(72,125),b=mapBounds('cave');morse.visible=false;scarySound(chance(.5)?'static':'whisper',true);
  setTimeout(()=>{if(!morse)return;morse.x=clamp(target.x+Math.cos(ang)*dist,b.x1,b.x2);morse.y=clamp(target.y+Math.sin(ang)*dist,b.y1,b.y2);morse.vx=morse.vy=0;morse.visible=true;morse.flash=1;morse.target=target;morse.targetUntil=time+4;scarySound('scrape',true);addChatMessage('Morse',pick(['... --- ...','..-. --- .-.. .-.. --- .--','.-. ..- -.']),'bot');if(target!==player)panicForMorse(target,0);},rand(420,900));
}
...
function updatePlayer(dt){
  ...
  const armScale=(player.mods.longArms||player.mods.morseForm)?1.85:1,reach=78*armScale;player.left.x=lerp(player.left.x,-42+lx*reach,.22);player.left.y=lerp(player.left.y,28+ly*reach,.22);player.right.x=lerp(player.right.x,42+rx*reach,.22);player.right.y=lerp(player.right.y,28+ry*reach,.22);
  ...
  if(player.mods.morseForm&&time>=playerMorseNextSound){playerMorseNextSound=time+rand(4.5,8.5);if(chance(.58))morseBeep();else scarySound(pick(['whisper','static','scrape','drone']),true);botsHere().forEach(b=>{if(Math.hypot(b.x-player.x,b.y-player.y)<650){b.knowsMorse=true;b.fear=Math.max(b.fear,1.1);if(!b.tagger&&!b.infected)b.state='flee';}});}
...
function updateMorse(dt){
  ...
  if(morse.nextSound<=0){morse.nextSound=rand(1.4,3.7);if(chance(.5))morseBeep();else morseSoundboard();}
  ...
  if(morse.nextAction<=0){
    morse.nextAction=rand(1.8,3.9);const victim=chooseMorseTarget(),roll=Math.random();morse.lastTarget=morse.target;morse.target=victim;morse.targetUntil=time+rand(2.2,4.4);
    if(roll<.18)morseKickTarget(victim);
    else if(roll<.36)morseBanTarget(victim);
    else if(roll<.54)morseGhostAbility(victim);
    else if(roll<.70)morseInvisibleAbility(victim);
    else if(roll<.84)morseStalkTarget(victim);
    else morseSoundboard();
    if(victim!==player&&chance(.62))panicForMorse(victim,rand(80,260));
    if(chance(.18))setTimeout(()=>{if(morse)morseSoundboard()},rand(450,1100));
  }
...
function drawPlayer(){
  const lw=handWorld(player.left),rw=handWorld(player.right),morseForm=player.mods.morseForm;
  const alpha=player.mods.invisible?.17:player.mods.ghost?.4:1;ctx.save();ctx.globalAlpha=alpha;
  const bodyColor=morseForm?'#000000':player.color;
  ctx.fillStyle='rgba(0,0,0,.22)';ctx.beginPath();ctx.ellipse(player.x+6,player.y+14,34,22,0,0,TAU);ctx.fill();
  ctx.strokeStyle=bodyColor;ctx.lineWidth=15;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(player.x-12,player.y+3);ctx.lineTo(lw.x,lw.y);ctx.moveTo(player.x+12,player.y+3);ctx.lineTo(rw.x,rw.y);ctx.stroke();
  ctx.fillStyle=bodyColor;ctx.beginPath();ctx.arc(player.x,player.y+8,28,0,TAU);ctx.fill();ctx.beginPath();ctx.arc(player.x,player.y-18,20,0,TAU);ctx.fill();for(const h of [lw,rw]){ctx.beginPath();ctx.arc(h.x,h.y,12,0,TAU);ctx.fill()}
  ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(player.x-7,player.y-22,4,0,TAU);ctx.arc(player.x+7,player.y-22,4,0,TAU);ctx.fill();if(!morseForm){ctx.fillStyle='#1b2a20';ctx.beginPath();ctx.arc(player.x-7,player.y-22,2,0,TAU);ctx.arc(player.x+7,player.y-22,2,0,TAU);ctx.fill();}
  ...
  if(!morseForm)drawOutfit(player.x,player.y,save.equipped,1.08);else{text('000',player.x,player.y+10,13,'#d45a66');text('... --- ...',player.x,player.y-60,10,'#d05d63');}
  text(morseForm?'Morse':save.playerName,player.x,player.y+59,11,morseForm?'#d5d5d5':'#fff');
  if(player.chat&&player.chatUntil>time&&$('#chatPanel').classList.contains('hidden')){const w=clamp(player.chat.length*6+18,80,300);ctx.fillStyle='rgba(5,12,9,.9)';roundedRect(player.x-w/2,player.y-110,w,38,10);ctx.fill();text(player.chat,player.x,player.y-91,10,'#fff')}
  if(player.mods.morseForm)text('BE MORSE',player.x,player.y+76,10,'#ff7782');if(player.mods.ghost)text('GHOST',player.x,player.y+(morseForm?89:76),10,'#b7eaff');if(player.mods.invisible)text('INVISIBLE',player.x,player.y+(morseForm?102:89),10,'#e8e8ff');
  if(player.mods.kickGun||player.mods.banGun){ctx.strokeStyle=player.mods.banGun?'rgba(190,100,255,.9)':'rgba(255,90,90,.85)';ctx.lineWidth=3;ctx.setLineDash([10,8]);ctx.beginPath();ctx.moveTo(rw.x,rw.y);ctx.lineTo(rw.x+Math.cos(player.angle)*700,rw.y+Math.sin(player.angle)*700);ctx.stroke();ctx.setLineDash([])}ctx.restore();
}
...
window.__canopyTest={setMap,spawnMorse:()=>{save.room='666';setMap('cave');morseRollDone=true;spawnMorse();persist();},join666:()=>{save.room='666';morse=null;morseRollDone=false;persist();bots=[];fillBots();setMap('cave');resetMode();},say:playerSpeak,openChat:()=>{$('#chatPanel').classList.remove('hidden');},openShop:()=>{$('#panel').classList.remove('hidden');$('.tab[data-tab="shop"]').click();},openSettings:()=>{$('#panel').classList.remove('hidden');$('.tab[data-tab="settings"]').click();},morseKickBot:()=>{if(!morse)spawnMorse();const b=pick(botsHere('cave'));if(b)morseKickTarget(b);},morseBanBot:()=>{if(!morse)spawnMorse();const b=pick(botsHere('cave'));if(b)morseBanTarget(b);},morseGhost:()=>{if(!morse)spawnMorse();morseGhostAbility(chooseMorseTarget());},morseInvisible:()=>{if(!morse)spawnMorse();morseInvisibleAbility(chooseMorseTarget());},morseForm:()=>{player.mods.morseForm=true;playerMorseNextSound=time+.1;syncModButtons();}};
